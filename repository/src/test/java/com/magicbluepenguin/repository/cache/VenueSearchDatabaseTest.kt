...
class VenueSearchDatabaseTest {
    ...
    fun `store and retrieve locations for queries`() = runBlocking {
        val expectedVenues = listOf(
            Venue("abc123", "Venue 1", "a b c"),
            Venue("poi098", "Venue 2", "x y z"),
            Venue("zxc345", "Venue 3", "1 2 3")
        ).sortedBy { it.id }
        ...
        assertEquals(expectedVenues, venuesForQuery.venues.sortedBy { it.id })
    ...
    fun `venue update is reflected in retrieved values`() = runBlocking {
        ...
        val initialVenue = Venue(commonVenueId, "Venue 1", "a b c")
        val updateVenue = Venue(commonVenueId, "Venue 1", "x y z")
